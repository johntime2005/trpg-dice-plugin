...
from .story_engine import SceneMood, StoryEngine, TensionLevel
...
def register_enhanced_prompts(plugin, story_engine: StoryEngine, custom_rules=None, rule_modeler=None):
    @plugin.mount_prompt_inject_method(name="narrative_guidance", description="根据剧情状态动态调整叙事风格")
    async def _inject_narrative(_ctx) -> str:
        return await inject_narrative_guidance(_ctx, story_engine)

    @plugin.mount_prompt_inject_method(name="improvisation_guide", description="即兴创作和场景描述指导")
    async def _inject_improv(_ctx) -> str:
        return await inject_improvisation_guide(_ctx)

    @plugin.mount_prompt_inject_method(name="check_philosophy", description="智能检定时机和失败处理哲学")
    async def _inject_checks(_ctx) -> str:
        return await inject_check_philosophy(_ctx)

    @plugin.mount_prompt_inject_method(name="dynamic_difficulty", description="根据剧情紧张度动态调整难度")
    async def _inject_difficulty(_ctx) -> str:
        return await inject_dynamic_difficulty(_ctx, story_engine)

    @plugin.mount_prompt_inject_method(name="npc_interaction", description="NPC 互动和对话生成指导")
    async def _inject_npc(_ctx) -> str:
        return await inject_npc_interaction_guide(_ctx)

    if custom_rules:

        @plugin.mount_prompt_inject_method(name="custom_rules", description="注入自定义规则和属性")
        async def _inject_custom(_ctx) -> str:
            return await custom_rules.get_custom_rules_prompt(_ctx.chat_key)

    if rule_modeler:

        @plugin.mount_prompt_inject_method(name="intelligent_rules", description="AI 智能理解和应用规则")
